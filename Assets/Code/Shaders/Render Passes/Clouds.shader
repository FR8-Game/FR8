Shader "Unlit/Clouds"
{
    Properties
    {
        ...
        [MainColor]
        _ColorHigh("Cloud Color High", Color) = (1.0, 1.0, 1.0, 1.0)
        _ColorLow("Cloud Color Low", Color) = (0.2, 0.2, 0.2, 1.0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                float4 pixel = float4(baseColor.rgb * lightColor * saturate(shading), saturate(baseColor.a * alpha));
                float4 fog = getFogColorWS(start.xyz);
                pixel = lerp(pixel, fog, fog.a);
